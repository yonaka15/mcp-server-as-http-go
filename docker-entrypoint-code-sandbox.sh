...
# MCP HTTP Server - Code Sandbox Specific Entrypoint
# Ensures Docker socket access for code-sandbox-mcp

set -e

echo "=== MCP HTTP Server Startup (Code Sandbox) ==="
echo "Current user: $(whoami) (UID: $(id -u))"

# Must run initial setup as root
if [ "$(id -u)" = "0" ]; then
    echo "🔧 Configuring Docker socket permissions for code-sandbox..."
    ...
    if [ -S "/var/run/docker.sock" ]; then
        ...
        echo "❌ Docker socket is required for code-sandbox MCP server"
        exit 1
    fi
fi

echo "🚀 Starting Code Sandbox MCP HTTP Server as $(whoami)..."
exec "$@"
